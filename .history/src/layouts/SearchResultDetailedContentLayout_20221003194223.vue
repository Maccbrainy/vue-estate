<template>
  ...
  <teleport v-if="isMediaTable" to="body">
    <section 
      class="fixed w-full h-screen bg-gray-800 top-0 right-0 left-0 z-30">
      <div class="w-11/12 h-5/6 bg-white m-auto relative -bottom-16">
        <close-mobile-menu class="absolute m-1 right-0 border border-transparent hover:border-teal-light text-teal" v-on:click="isMediaTable = false" />
        <div class="w-8/12">
          <div class="flex justify-between">
            <div>Media Photo</div>
            <div>Map</div>
            <div>Schools</div>
            <div>Shop and Eat</div>
          </div>
          <div class="bg-gray-200 h-auto"></div>
        </div>
...
export default {
  ...
  setup(props) {
    ...
    watchEffect(()=> {
      console.log("Model is:", isMediaTable.value);
    })
