...
export default {
  ...
  setup(props) {
    ...
    watchEffect(async () => {
      ...
      contextProvider.value.status = removeUnderScoresFromAString(propertyDetail.value[0].prop_status);
      contextProvider.value.clientFlags = propertyDetail.value[0].client_display_flags;
      contextProvider.value.lastUpdated = propertyDetail.value[0].last_update;
