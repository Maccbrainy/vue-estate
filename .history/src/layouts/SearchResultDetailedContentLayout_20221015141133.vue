          ...
          >
            ...
            <div id="#RequestForTourModule" class="text-center">
              ...
              <button
                ...
                :key="request"
                v-on:click="getScheduleORRequestModalToOpen(event, request)"
                type="button"
                class="
                  w-full
                  border 
    ...
    >
      <div ref="isModalOpenRef" class="w-5/12 h-5/6
        ...
        <div class="
            ...
            <widget-schedule-tour v-if="isModalOpen && isScheduleTour"
              v-bind:propertyName="fullPropertyAddressIdentity"> 
             </widget-schedule-tour>
            <widget-request-info-form  v-if="isModalOpen && !isScheduleTour"
            v-bind:propertyName="fullPropertyAddressIdentity">
          </widget-request-info-form>
          </div>
          <close-mobile-menu
...
export default {
  ...
  setup(props) {
    ...
    const getScheduleORRequestModalToOpen =(_, request) => {
      isModalOpen.value = !isModalOpen.value;
      if (request == "Shedule a tour"){
        isScheduleTour.value = true;
      }
      if (request == "Request for info"){
        isRequestInfo.value = true;
      }
    }
