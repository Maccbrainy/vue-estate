        ...
        <div id="#contactModule" class="w-4/12 mt-8 max-h-96 relative">
          <div
            class="
              ...
              shadow-2xl
            ">
            <div class="contact-form text-gray-700">
              <h2 class="text-base font-semibold capitalize">Contact This Property</h2>
              <h3 class="flex flex-col my-3 text-sm">
                <span class="font-semibold">Leasing Agent</span>
                <span>(510) 288-5187</span>
              </h3>
              <div
                v-bind:class="{
                  'outline-none ring ring-offset-2 ring-offset-white ring-teal-light': focused
                }"
                class="relative border border-gray-500 rounded-lg pt-1">
                <label v-bind:class="{
                   'transform translate-y-2/4 text-base': !focused && !getContactValues,
                   'font-semibold text-xs text-gray-700': focused || getContactValues,
                  }" class="absolute text-gray-400 left-4">Name</label>
                <input
                  v-model="getContactValues"
                  v-on:focus="focused = true"
                  v-on:blur="focused = false"
                  v-bind:placeholder="focused ? 'Your Name' : ''"
                  type="name"
                  v-bind:class="{'transform translate-y-2': focused || getContactValues}"
                  class="outline-none w-full bg-transparent py-4 px-4"
                />
              </div>
              <div class="flex text-red-600 text-xs p-1 items-center">
                <information-icon/>
                <span class="px-1">Please enter your first and last name.</span>
              </div>
            </div>
  ...
  <teleport v-if="isMediaTable" to="body">
    ...
    >
      ...
      >
        ...
        <div
          class="relative w-8/12 h-full top-24 -bottom-8 overflow-y-auto ml-1"
        >
...
export default {
  ...
  setup(props) {
    ...
    const focusedCount = computed(() => {
      let count = 0;
      return focused.value ? count++ : count;
    });

    watchEffect(() => {
      console.log("Focused count:", focusedCount.value);
    });
