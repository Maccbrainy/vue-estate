<template>
  <div
    class="w-full relative top-16 bg-white overflow-x-hidden min-h-screen"
  >
    <div
    ...
    <div class="w-9/12 mx-auto px-4">
          ...
          >
            <div v-if="renderIsScheduleFormType">
              <div class="flex flex-initial gap-16 items-center">
                ...
                <div>
                  <widget-schedule-tour> </widget-schedule-tour>
                </div>
              </div>
            </div>
            <widget-contact-form
      ...
      <div id="#ListingProviderModule" class="relative my-6">
        <h2 class="relative text-gray-700 font-bold text-xl my-4">
          {{`Listing provided by`}}
        </h2>
        <p></p>
      </div>
      <div v-if="bedProperty" id="#propertyDescriptionModule" class="relative my-4">
        <p class="text-xs text-gray-500 font-normal text-justify">
          {{`${propertyAddress}, ${propertyCity}, ${slug} ${postalCode} is a ${bedProperty == 0 ? 'Studio,' : `${bedProperty} bedroom,`} ${bathProperty > 0 ? `${bedProperty} bathroom,` : ""} ${propertySqft != null ? `${propertySqft} sqft,` : ""} ${propertyDetail[0].prop_type} built in ${propertyDetail[0].year_built}. 298 Saint Marks Ave is located in Prospect Heights, Brooklyn. This property is currently available for sale and was listed by StreetEasy on Sep 9, 2022.`}}
        </p>
      </div>
    </div>
  </div>
