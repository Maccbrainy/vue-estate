<template>
  ...
  <teleport v-if="isMediaTable" to="body">
    ...
    >
      <div 
        ref="mediaTableRef"
        class="w-11/12 h-5/6 bg-white m-auto relative -bottom-14 rounded-lg p-4 border-b"
      >
        <close-mobile-menu
          class="absolute right-2 top-2 hover:text-teal text-gray-600"
        ...
        <div class="relative w-8/12">
          <div>
            ...
            >
              <li
                v-for="(title, index) in ['Media','Map', 'Schools', 'Shop & Eat']"
                :key="index"
              >
                <button
                  v-on:click="getMappingIdForContent"
