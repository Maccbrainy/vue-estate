...
export default {
  ...
  setup(props) {
    ...
    const moveSlide = (direction) => {
      ...
      let moveIndex = (contentWidthTheVisibleArea - overAllScrollWidthLength); 
      if (direction === 1){
        moveIndexDirection-=moveIndex;
      }
      if (direction === -1){
        moveIndexDirection+=moveIndex;
      }
      ...
      dateItemRefs.value.forEach((date) => {
        date.$el.style.transform = `translateX(${moveIndexDirection}px)`
      });
