...
export default {
  ...
  setup(props) {
    ...
    const getScheduleORRequestModalToOpen =(_, requestType) => {
      ...
      console.log('First contentRef:', isModalOpenRef.value);
      // useOnOutsideClicks(isModalOpenRef.value, () => isModalOpen.value = false);

      const closeListener = (event) => {
        if (!isModalOpenRef.value || isModalOpenRef.value.contains(event.target)){

      console.log('second contentRef:', isModalOpenRef.value);
