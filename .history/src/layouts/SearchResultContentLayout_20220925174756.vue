        ...
        >
          ...
          <div class="pt-1 mb-1">
            ...
            >
              ...
              >
                <span class="-ml-1">{{ activeFilterEntry }}</span>
              </li>
            </ul>
...
export default {
  ...
  setup(props) {
    ...
    watchEffect(() => {
      ...
      console.log("===ACTIVEROUTEPATH===:", updateRouterParams.activeRouteTab);
      console.log(
        "===ACTIVEROUTEPATH SEARCHEDDATA===:",
        storeData.value.searchedData.activeRouteTab
      );
      ...
      if (
        ...
        if (storeData.value.searchedData.activeRouteTab != "HomePage") {
          // updateRouterParams.activeRouteTab = storeData.value.activeRoutePath;
          updateRouterParams.activeRouteTab = 
            storeData.value.searchedData.activeRouteTab;
          updateRouterParams.state_code =
            storeData.value.searchedData.state_code;
          updateRouterParams.city = storeData.value.searchedData.city;

          store.commit("setUseRouterPush", updateRouterParams);
          console.log(
            "002filterIsNotActive && NotSalesNearMe AND NotRentsNearMe SEARCHEDDATA", storeData.value.searchedData.activeRouteTab
          );
          console.log(
            "002filterIsNotActive && NotSalesNearMe AND NotRentsNearMe", storeData.value.activeRoutePath
          );
        }
      }
      if (
    ...
    const activeFilterDescriptionEnteries = computed(() => {
      ...
      let filterEntries = {
        bed:
          storeData.value.filterDescriptionInfo.bedroom == "All Beds"
            ? ""
            : storeData.value.filterDescriptionInfo.bedroom,
        bath:
          storeData.value.filterDescriptionInfo.bathroom == "Any"
            ? ""
            : storeData.value.filterDescriptionInfo.bathroom,
        priceRange:
        ...
        homeType:
          ...
          storeData.value.filterDescriptionInfo.homeType == "All Rental Types"
            ? ""
            : storeData.value.filterDescriptionInfo.homeType,
        // dogsAllowed: storeData.value.filterDescriptionInfo.dogsAllowed,
        // catsAllowed: storeData.value.filterDescriptionInfo.catsAllowed,
        homeFeatures:
          storeData.value.filterDescriptionInfo.features ==
            "All Home Features" ||
          storeData.value.filterDescriptionInfo.features ==
            "All Rental Features"
            ? ""
        ...
        // lotSize: storeData.value.filterDescriptionInfo.lotSize,
        // hasOpenHouses: storeData.value.filterDescriptionInfo.hasOpenHouses,
        // has3DTours: storeData.value.filterDescriptionInfo.has3DTours,
        // foreClosure: storeData.value.filterDescriptionInfo.foreClosure,
        // newConstruction: storeData.value.filterDescriptionInfo.newConstruction,
        // newPlans: storeData.value.filterDescriptionInfo.newPlans,
        // radius: storeData.value.filterDescriptionInfo.radius,
        // contingents: storeData.value.filterDescriptionInfo.contingents,
      };
    ...
    onMounted(() => {
      let activeRoutePath =
        props.name == "RentsNearMe"
          ...
          ? "BuyPage"
          : props.name;
      store.commit("setActiveRoutePath", activeRoutePath);
    ...
    // onBeforeUpdate(() => {
    //   const { listingsByOthers, listingsByAgent } =
    //     useSortListingsByAgentAndOthers(storeData.value.allPropertyListings);
    //   const { listTitle } = useListingTitle(props.name, props.slug, props.city);

    //   listingTitle.value = listTitle.value;
    //   propertyListingByAgent.value = listingsByAgent.value;
    //   propertyListingByOthers.value = listingsByOthers.value;
    //   console.log("DO YOU REACT IN ALL CHANGES, YES onBeforeUpdate");

    // });
    onBeforeUnmount(() => {
      // store.commit("setSearchedData", "");
      store.commit("setIsLoading", false);
    });
    return {
      // setData,
