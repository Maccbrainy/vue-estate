        ...
        <div id="#contactModule" class="w-4/12 mt-8 max-h-96 relative">
          <div
            ...
            >
              ...
              <div 
                ref="dateContainerWidthRef"
                class="relative outline-none"
              >
              index move left: {{showHideButtonLeft}}
              index move right: {{showHideButtonRight}}
              index move left++: {{-(showHideButtonLeft)}}
                <button
                  v-show="!showHideButtonLeft == 0"
                ...
                <button
                  v-show="showHideButtonLeft== 0 || showHideButtonRight "
                ...
                <ul
                  ...
                  <li
                    class="w-1/4"
                    v-for="(date, index) in getDatesRangeInAWeek"
                    :key="date"
                    v-bind:ref="el => dateItemRefs[index] = el"
                  >
                    <button
                      v-bind:class="{
                        'border-2 border-teal-light hover:border-teal-light': index == 0,
                        'border': index != 0,
                      }"
                      class="
                        border-gray-700
                    ...
                    >
                      ...
                      <div class="font-bold text-base">{{date.dayOfMonth}}</div>
                      <div>{{date.month}}</div>
                    </button>
                  </li>
                </ul>
    ...
    >
      ...
      >
        ...
        <div class="            
            ...
            border
            px-2
            rounded-tl-lg rounded-bl-lg shadow-2xl transform -translate-y-96 top-24 -mt-0.5">
          <div class="w-11/12 m-auto pt-2">
            <h2 class="text-gray-700 text-lg font-semibold pb-2">Contact This Property</h2>
            <widget-contact-form></widget-contact-form>
          </div>
        </div>
      </div>
...
export default {
  ...
  setup(props) {
    ...
    const dateContainerWidthRef = ref(null);
    const dateContentWidthRef = ref(null);
    const dateItemRefs = ref([]);

    const moveSlideDirectionIndex = ref(0);
    const showHideButtonLeft = ref(0);
    const showHideButtonRight = ref(true);
    ...
    const getDatesRangeInAWeek = computed(() => {
      const dates = [];

      const startDate = new Date();
      const endDate = addDays(new Date(), 25);
    ...
    const moveSlide = (direction) => {
      ...
      console.log("Getref:", dateContainerWidthRef.value);
      console.log("GetclientWidth:", dateContainerWidthRef.value.clientWidth);
      console.log("GetoffsetWidth:", dateContainerWidthRef.value.offsetWidth);
      console.log("GetclientWidth content:", dateContentWidthRef.value.clientWidth);
      console.log("GetoffsetWidth content:", dateContentWidthRef.value.offsetWidth);
      console.log("GetscrollWidthContent:", dateContentWidthRef.value.scrollWidth);
      // console.log("GetAllclientWidthItemRef:", dateItemRefs.value.clientWidth);

      dateItemRefs.value.forEach( (date, index) => {
        let width = date.offsetWidth;
        // let width2 = date.clientWidth;
        console.log(`GetoffsetWidthItemRef ${index+=1}:`, width);
        // console.log("GetclientWidthItemRef:", width2);
      });
      ...
      let moveIndex = (overAllScrollWidthLength - contentWidthTheVisibleArea); 
      if (direction === 1){
        moveSlideDirectionIndex.value-=moveIndex;
      }
      if (direction === -1){
        moveSlideDirectionIndex.value+=moveIndex;
      }
      ...
      showHideButtonLeft.value = moveSlideDirectionIndex.value;

      let setMoveSlideDirectionIndexToPositiveValue = -(moveSlideDirectionIndex.value);
      let remainderOfOverAllScrollWidthLength = overAllScrollWidthLength - setMoveSlideDirectionIndexToPositiveValue;
      showHideButtonRight.value = remainderOfOverAllScrollWidthLength > contentWidthTheVisibleArea ? true : false;

      // console.log("moveSlideDirectionIndex:", moveSlideDirectionIndex.value);
      // console.log("contentWidthTheVisibleArea:", contentWidthTheVisibleArea);
      // console.log("overAllScrollWidthLength:", overAllScrollWidthLength);
      // console.log("RemainderScrollWidth:", (overAllScrollWidthLength - showHideButtonRight.value));
    };

    watchEffect(() => {
      console.log("Data Range", getDatesRangeInAWeek.value);
    });
