<template>
  ...
  <teleport v-if="isMediaTable" to="body">
    ...
    >
      <div class="w-11/12 h-5/6 bg-white m-auto relative -bottom-16 rounded-lg">
        <close-mobile-menu
          class="
            absolute
            m-1
        ...
        />
        <div class="relative w-8/12 top-2">
          <div>
            <ul
              class="
                flex
            ...
            >
              <li
                v-for="title in ['Media Photo','Map', 'Schools', 'Shop & Eat']"
                :key="title"
              >
                <button
                  v-on:click="getMappingIdForContent"
            ...
            <div class="block w-full h-96 rounded-lg bg-gray-200 m-4">
              <span class="p-4">{{ renderMappingContent }}</span>
            </div>
          </div>
        </div>
