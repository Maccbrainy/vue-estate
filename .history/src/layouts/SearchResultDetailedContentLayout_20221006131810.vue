  ...
  <teleport v-if="isMediaTable" to="body">
    ...
    >
      <div
        ...
        "
      >
        <div class="flex justify-between items-center border-b px-4 pt-2 pb-3">
          <ul
            class="
              flex
              justify-start
          ...
          >
            ...
            >
              <button
                ...
                type="button"
                class="rounded-md text-base font-medium text-gray-700 px-5 py-2"
              >
                {{ title }}
              </button>
            </li>
          ...
          <div>
            {{
              `${propertyAddress} | $${propertyPrice} | ${
                bedProperty == 0 ? "Studio" : bedProperty > 0 ? bedProperty : ""
              } ${bathProperty > 0 ? bathProperty : ""}`
            }}
          </div>
        </div>
        <div class="relative w-8/12 overflow-y-scroll">
          <div class="block w-full rounded-lg bg-gray-200 ml-3">
            <div class="text-gray-700 text-lg font-semibold py-3">
              {{ `Listing Photos (${propertyPhotos.length})` }}
            </div>
            <div class="grid grid-cols-12 gap-0 pb-2">
              <div 
                v-for="(photo, index) in propertyPhotos" :key="index"
                v-bind:class="{
                  'w-1/2 col-start-1 col-end-7': index == 0,
                  'w-1/2 col-start-7 col-end-13': index == 1,
                }" 
                class="flex overflow-hidden p-4 image-height">
                <picture>
                  <img
