<template>
  <li>
    <div ref="buttonRef" class="mr-2" v-on:click="buttonIsOpen = !buttonIsOpen">
      <button-slot 
        v-bind:class="{ 
  ...
  setup() {
    ...
    function listener(event){
      if (!fieldSetRef.value || fieldSetRef.value.contains(event.target)){
        console.log("000000000FIELDSETREF");
        return;
     } 
      if (!buttonRef.value || buttonRef.value.contains(event.target)){
        console.log("00000000BUTTONREF");
        return;
     }
    ...
    //   if (!buttonRef.value || buttonRef.value.contains(event.target)){
    //     console.log("00000000BUTTONREF");
    //     return buttonIsOpen.value = !buttonIsOpen.value;
    //  }
    //   buttonIsOpen.value = false;
    // };
    onMounted(() => {
