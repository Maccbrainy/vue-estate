  ...
  setup() {
    ...
    function close(){
      ...
      const listener = (event) => {
        ...
        if (catchOutsideClick(event, fieldSet.value)) {
  
          window.removeEventListener("click", listener);

          console.log("THTHTHTHTH:", fieldSet.value.contains(event.target));
          console.log("THTHTHTfieldSet:", fieldSet.value);
    ...
    const catchOutsideClick = (event, fieldSet) => {
      if (!fieldSet || fieldSet.contains(event.target)){
        console.log("00001", fieldSet.contains(event.target))
        return;
      }
      // if (buttonIsOpen.value == true && fieldSet.contains(event.target)){
      //   console.log("00002", buttonIsOpen.value ,fieldSet.contains(event.target))
      //   return false;
      // }
      // if (buttonIsOpen.value == false && !fieldSet.contains(event.target)){
      //   console.log("00003", buttonIsOpen.value , !fieldSet.contains(event.target))
      //   return true;
      // }
    }
