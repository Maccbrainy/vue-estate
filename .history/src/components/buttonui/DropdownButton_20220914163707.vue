...
export default {
  ...
  methods: {
    toggleButtonOpenClose(event){
      ...
      console.log("FFFFFFF SET CURRENT:", this.fieldSet.current);
      console.log("FFFFFFF SET CURRENT status:", this.fieldSet.current.contains(event.target));
      console.log("FFFFFFF SET  event:", event.target);
      // const listener = () => {
      //   if (event.target != this.fieldSet) {
