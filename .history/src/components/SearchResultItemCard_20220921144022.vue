      ...
      <div v-bind:id="home.property_id">
        <router-link 
          ...
          <widget-client-flags 
            ...
            v-bind:isNewLists="isNewListing"
            v-bind:hasOpenHousesTime="hasOpenHouses"
            v-bind:hasTour3D="has3Dtour"
            v-bind:isNewPlanNotYetBuilt="isNewPlan"
            v-bind:isPetsFriendly="petsFriendly"
            v-bind:isRental="isRentalProperty"
            v-bind:lastUpdateTime="propertyLastUpdated"
            >
          </widget-client-flags>
          <!-- <div 
            v-show="!storeData.isLoading && !storeData.fetchingIsBusy" 
    ...
    const hasOpenHouses = computed(() => {
      ...
      if (getValueIfPropertyExist){
        let startDate = format(
          new Date(props.home.open_houses[0].start_date),
          "E,h"
        );
        let endDate = format(
          new Date(props.home.open_houses[0].end_date),
          "ha"
        );
