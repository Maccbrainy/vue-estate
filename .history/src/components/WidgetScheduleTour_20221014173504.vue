<template>
  <div class="relative my-4 w-full">
    <ul class="flex items-center justify-evenly">
      ...
      >
        <button
          v-on:click="getScheduleTourType(event, title)"
          :id="title"
          type="button"
          class="
            flex
          ...
          <div
            :class="{
              'absolute w-1/2 py-2 h-full px-5 bg-white border rounded-lg':
                title == inPersonOrVideoChatScheduleType,
              'transform translate-x-0': inPersonOrVideoChatScheduleType == `In-Person`
            }"
          ></div>
...
export default {
  ...
  setup() {
    ...
    const getScheduleTourType = (_, title) => {
    ...
    watchEffect(() => {
      console.log("Tour Type:", inPersonOrVideoChatScheduleType.value);
    })
