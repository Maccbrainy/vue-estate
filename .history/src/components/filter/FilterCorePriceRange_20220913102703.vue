...
export default {
  ...
  setup() {
    ...
    const getPriceMinTitle = computed(() =>{
      let data = priceRanges.value.filter(
        (range) => range.id == selectedMin.value);
      return data[0].title;
    });
    const getPriceMaxTitle = computed(() =>{
      let data = priceRanges.value.filter(
        (range) => range.id == selectedMax.value);
      return data[0].title;
    });
    ...
    watchEffect(() => {
      ...
      let minPriceTitle = getPriceMinTitle.value;
      let maxTitle = getPriceMaxTitle.value;
      console.log("PRICE TITLE MIN WATCH EFFECT:", minPriceTitle)
      console.log("PRICE TITLE MAX WATCH EFFECT:", maxTitle)
