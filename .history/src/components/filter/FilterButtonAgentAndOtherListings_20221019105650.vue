<template>
  <div v-show="isBuyPage" class="relative flex justify-between">
    ...
    >
      <button
        v-on:click="getScheduleTourType(event, type, arrayIndex)"
        type="button"
  ...
  <switch-button-container
    v-show="isBuyPage"
    v-bind:class="{
      'border border-gray-300': storeData.isLoading,
  ...
  >
    ...
    >
      <switch-button
        class="pr-4"
        :id="agentType[0].id"
        v-bind:class="{ 'text-gray-400': storeData.isLoading }"
        v-bind:disabled="storeData.isLoading"
      >
        <span>{{ agentType[0].title }}</span>
        <span v-show="!storeData.isLoading" class="xxs:hidden font-normal ml-1"
...
export default {
  ...
  setup() {
    ...
    const getScheduleTourType = (_, type, arrayIndex) => {
