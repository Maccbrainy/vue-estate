...
export default {
  ...
  setup() {
    ...
    // const priceMinRange = computed(
    //   () => storeData.value.propertyFilters.priceMinRange || "0");
    // const priceMaxRange = computed(
    ...
    onBeforeUpdate(() => {
      console.log("PRICEPRICEPRICE BEFORE UPDATE:")
      store.commit("setMinPriceRange", null);
      store.commit("setMaxPriceRange", null);
      store.commit("setPriceRangeInfo", "Any Price");
    });
    return {
      routeNames,
      priceRanges,
