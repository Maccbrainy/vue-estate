...
export default {
  ...
  setup() {
    ...
    const fromRentsNearOrSalesNearMe = ref(Boolean);
    ...
    onBeforeRouteLeave((to, from) => {
      if (from.name == "RentsNearMe" || from.name == "SalesNearMe") {
        fromRentsNearOrSalesNearMe.value = true;
        console.log("RentsNearMeORSalesNearMe @onBeforeRouteLeave:", fromRentsNearOrSalesNearMe.value);
      }
    });
    onBeforeUnmount(() => {
      //Reset propertyFilter vuex store state to default from here
      //Except if route name is HomesNearMe or RentsNearMe
      if (fromRentsNearOrSalesNearMe.value){
        store.dispatch("resetFilterPriceRange");
        console.log("SORRY PRICECORE IS BEEN DESTROYED");
        console.log("RentsNearMeORSalesNearMe is:", fromRentsNearOrSalesNearMe.value);
      }
    });
    return {
      routeNames,
