  ...
  setup() {
    ...
    watchEffect(() => {
      if (storeData.value.activeRoutePath || priceRanges.value) {
        ...
        let minPriceTitle = getPriceRangeDataTitle(priceRanges.value, selectedMin.value);
        let maxTitle = getPriceRangeDataTitle(priceRanges.value, selectedMax.value);
    ...
    onBeforeMount(() => {
      ...
      console.log("THIS IS PRICE ONBEFOREMOUNT:", route);
    });
    onBeforeUnmount(() => {
      //Here i have to clear vuex store state from here
      //Except route name is HomesNearMe or RentsNearMe
      // if ( route.name != "RentsNearMe" || route.name != "SalesNearMe")
      store.dispatch("resetFilterPriceRange");
      // selectedMin.value = 0;
      // selectedMax.value = 0;
      console.log("SORRY PRICECORE IS BEEN DESTROYED");
      console.log("SORRY PRICECORE RESETTED $ ROUTE NAME:", route.name);
      console.log("DESTROY PRICE ROUTER:", router);
    });
    onBeforeRouteLeave(() => {
      console.log("I AM BEHAVING JUST LIKE ONBEFOREUNMOUNTED INSTANCE")
    })
    return {
      routeNames,
      priceRanges,
