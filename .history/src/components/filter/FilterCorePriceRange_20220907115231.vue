    ...
    <dropdown-button-select-box>
      <dropdown-button-select v-model="selectedMin">
        <dropdown-button-select-option 
          ...
          {{ priceRange.id == null ? "No Min" : `$${priceRange.title}`}}
        </dropdown-button-select-option>
    ...
    <dropdown-button-select-box>
      <dropdown-button-select v-model="selectedMax"> 
        <dropdown-button-select-option 
          ...
          v-bind:key="priceRange.title" 
          v-bind:value="priceRange.id">
          {{ priceRange.id == null ? "No Max" : `$${priceRange.title}`}}
        </dropdown-button-select-option>
      </dropdown-button-select>
    </dropdown-button-select-box>
  </div>
...
export default {
  ...
  setup() {
    ...
    watchEffect(() => {
      let minPrice = selectedMin.value ? parseInt(selectedMin.value) : null;
      let maxPrice = selectedMax.value ? parseInt(selectedMax.value) : null;
    ...
    function getPriceRangeDataTitle(ranges, rangeValue){
      let defaultValue = rangeValue ? rangeValue : 0;
      let data = ranges.filter((range) => range.id == defaultValue);
      return data[0].title;
    }
