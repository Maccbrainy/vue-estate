...
export default {
  ...
  setup(props) {
    ...
    const getHomeTypeTitle = (types, theCheckedTypeId) => {
      ...
      console.log("getCoreFeature:", typeChecked);
    ...
    onBeforeMount(() => {
      if (route.query.homeFeatures) {
         ...
         * */
        let regEx = /\s*(?:,|$)\s*/;
        let homeFeatures = route.query.homeFeatures.split(regEx);
        store.commit("setHomeFeatures", homeFeatures);
        console.log("ONMOUNTED homeFeatures:", homeFeatures);
      }
      console.log(">>>>CORE FEATURES ONBEFOREMOUNT:", route);
    });
    return {
      checkedSignal,
      checkedFeatures,
