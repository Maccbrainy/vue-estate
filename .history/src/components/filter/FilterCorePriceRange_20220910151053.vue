...
export default {
  ...
  setup() {
    ...
    watchEffect(() => {
      let minPrice = 
        selectedMin.value == 0 ? "" : parseInt(selectedMin.value);
      let maxPrice = 
        selectedMax.value == 0 ? "" : parseInt(selectedMax.value);
    ...
    function getPriceRangeDataTitle(ranges, rangeValue){
      console.log("RANGE O:", rangeValue);
      console.log("RANGE Options:", ranges);
      let data;
      if (rangeValue){
        data = ranges.filter((range) => range.id == rangeValue);
        return data[0].title;
      }
    }
