...
export default {
  ...
  setup() {
    ...
    const isDogsAllowed = ref(storeData.value.propertyFilters.allowsDogs);
    const isCatsAllowed = ref(storeData.value.propertyFilters.allowsCats);
    const isRentPage = ref(Boolean);
    const routeNames = [...settingsData.routeNames];

    const petsTitle = computed(() => {
      return !isDogsAllowed.value && !isCatsAllowed.value 
        ? "Pets"
        : isDogsAllowed.value && isCatsAllowed.value
        ? "Pets (2)"
        : isDogsAllowed.value
        ? "Dog Pets"
        : "Cat Pets" 
