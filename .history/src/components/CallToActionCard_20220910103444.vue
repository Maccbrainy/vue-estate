    ...
    const setLongitudeAndLatitude = (e) => {
      navigator.geolocation.getCurrentPosition(
        ({ coords: { latitude, longitude } }) => {
          ...
          console.log("From CallToAction", e.target.id);
      ...
      // if (e.target.id == "SalesNearMe" || e.target.id == "RentsNearMe"){
      //   let searchPayload = {
      //     city: "San Francisco",
      //     state_code: "CA",
      //     activeRouteTab: e.target.id,
      //   };
      //   store.commit("setUseRouterPush", searchPayload);
      //   store.commit("setSearchedData", searchPayload);
      // } else {
      //   console.log("THIS IS THE NEIGHBOURHOOD!!! COMING SOON")
      // }
