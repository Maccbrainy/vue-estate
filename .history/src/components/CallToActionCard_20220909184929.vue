    ...
    const setLongitudeAndLatitude = (e) => {
      navigator.geolocation.getCurrentPosition(
        ({ coords: { latitude, longitude } }) => {
          ...
          console.log("From CallToAction user location", e.target.id);
          console.log("From CallToAction user location", e);
          // let payload = {
          ...
          let correspondingRoute = 
            e.target.id == "houses-for-sale-near-me "
              ? "RentsNearMe" 
              : e.target.id == "apartments-for-rent-near-me" 
              ? "SalesNearMe"
              : e.target.id
          router.push({
            name: correspondingRoute,
            params: {
              lat: userCordinates.lat,
              long: userCordinates.long,
            }
