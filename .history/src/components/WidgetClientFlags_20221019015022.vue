...
export default {
  ...
  setup() {
    const displayClientFlags = inject("displayClientFlags");

    const propertyStatus = computed(() =>
      displayClientFlags.status == "for_sale"
        ? "FOR SALE"
        : displayClientFlags.status == "for_rent"
        ? "FOR RENT"
        : false
    );
    const propertyLastUpdated = computed(() => {
      let todayDate = new Date();
      let lastUpdated = new Date(displayClientFlags.lastUpdates);
    ...
    const isNewListing = computed(() => {
      let getValueIfPropertyExist = Object.hasOwn(
        displayClientFlags.clientFlags,
        "is_new_listing") 
        ? displayClientFlags.clientFlags.is_new_listing 
        : false;
    ...
    const hasOpenHouses = computed(() => {
      let getValueIfPropertyExist = Object.hasOwn(
        displayClientFlags.clientFlags,
        "has_open_house") 
        ? displayClientFlags.clientFlags.has_open_house 
        : false;
      if (getValueIfPropertyExist){
        let startDate = format(
          new Date(displayClientFlags.clientFlags.open_houses[0].start_date),
          "E,h"
        );
        let endDate = format(
          new Date(displayClientFlags.clientFlags.open_houses[0].end_date),
          "ha"
        );
    ...
    const has3Dtour = computed(() => {
      let getValueIfPropertyExist = Object.hasOwn(
        displayClientFlags.clientFlags,
        "has_matterport") 
        ? displayClientFlags.clientFlags.has_matterport 
        : false;
      return getValueIfPropertyExist;
    });
    const isNewPlan = computed(() => {
      let getValueIfPropertyExist = Object.hasOwn(
        displayClientFlags.clientFlags,
        "is_new_plan") 
        ? displayClientFlags.clientFlags.is_new_plan 
        : false;
      return getValueIfPropertyExist;
    });
    const petsFriendly = computed(() => {
      let getCatValueIfPropertyExist = Object.hasOwn(
        displayClientFlags.clientFlags,
        "allows_cats") 
        ? displayClientFlags.clientFlags.allows_cats
        : false;
      let getDogValueIfPropertyExist = Object.hasOwn(
        displayClientFlags.clientFlags,
        "allows_dogs")
        ? displayClientFlags.clientFlags.allows_dogs 
        : false;
