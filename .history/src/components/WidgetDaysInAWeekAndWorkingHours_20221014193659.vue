<template>
  <div class="relative outline-none mt-1">
    ...
    <div class="pt-4">
      ...
      >
        <information-icon />
        <span class="px-1">Today's tour time has passed,check other days of your choice</span>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  setup(props, context) {
    ...
    const getTheDayInformation = (_, date, index) => {
      ...
      let workingHours = setWorkingHoursRangeInADate(date.rawDate);
      getDateWorkingHours.value =
        workingHours.length > 0 ? workingHours[0].date : `Time has Elapsed`;
      context.emit('update:modelValue', getDateWorkingHours.value);
      return;
    };
