      ...
      <thead>
        <tr class="w-full py-3 text-gray-600">
          <th 
            ...
            class="text-left py-2 px-4">{{ title }}</th>
        </tr>
      </thead>
      <tbody class="bg-white">
        <tr
          v-for="propertyHistory in propertyHistories"
          :key="propertyHistory.date || propertyHistory.tax"
          class="border w-full"
        >
          <td class="w-auto py-2 px-4">
            {{ propertyHistory.date || propertyHistory.year }}
          </td>
          <td class="w-auto py-2 px-4">{{ propertyHistory.price ||  getValueIfPropertyExist }}</td>
          <td class="w-auto py-2 px-4">
...
export default {
  ...
  setup(props) {
    ...
    const getValueIfPropertyExist = computed(() => {
      let valueProperty;
      if (props.propertyTitle == "Tax History"){
        valueProperty = props.propertyContentData.map((data) => {
          Object.hasOwn(data, "assessment");
          console.log("HAS PROPS inner:", data.assessment.building);
        })
        console.log("HAS PROPS inside:", valueProperty);
      }
      console.log("HAS PROPS: outside", valueProperty);
