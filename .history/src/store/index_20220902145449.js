...
export default createStore({
  ...
  mutations: {
    ...
    setUpdateRouteParams(_, routePayload){
      // state.isLoading = true;
      let priceQ = routePayload.price ? routePayload.price : "";
      let bedQ = routePayload.bed ? routePayload.bed : "";
      let homeTypeQ = 
        routePayload.homeType.length > 0 ? routePayload.homeType.join() : "";
      let bathQ = routePayload.bath ? routePayload.bath : "";
      console.log("queryLoad price:", priceQ);
      console.log("queryLoad bed:", bedQ);
      console.log("queryLoad homeType:", homeTypeQ);
      console.log("queryLoad bath:", bathQ);
      useRouterPush(routePayload);
      router.push({
        path: "/:slug/:city?/:price?/:bed?/:homeType?/:bath?",
        params: {
          slug: routePayload.slug,
          city: routePayload.city,
